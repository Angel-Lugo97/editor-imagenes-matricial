...
function obtenerDimensionesImagen(rutaImagen) {
   // 1. Leer solo el encabezado del PNG
  const buffer = fs.readFileSync(rutaImagen);
  const png = PNG.sync.read(buffer);
  
  // 2. Retornar las dimensiones de la imagen
  return { 
    ancho: png.width, 
    alto: png.height, 
...
function ajustarBrillo(matriz, factor) {
 // 1. Crear matriz resultado (copiar la matriz original)
  const resultado = matriz.map(fila => fila.map(pixel => ({
    r: pixel.r,
    g: pixel.g,
    b: pixel.b,
    a: pixel.a
  ...
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      // Multiplicar cada componente de color por el factor y limitar los valores
  ...
  // 6. Retornar la matriz con los valores ajustados
  return resultado;
...
function invertirColores(matriz) {
   // 1. Crear matriz resultado (copia de la matriz original)
  const resultado = matriz.map(fila => fila.map(pixel => ({
    r: pixel.r,
    g: pixel.g,
    b: pixel.b,
  ...
  // 2. Para cada pixel, invertir los valores de R, G, B
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      resultado[i][j].r = 255 - matriz[i][j].r; // Invertir el rojo
      resultado[i][j].g = 255 - matriz[i][j].g; // Invertir el verde
      resultado[i][j].b = 255 - matriz[i][j].b; // Invertir el azul
      // El canal alpha NO se modifica
    }
  }

  // 6. Retornar la matriz con los colores invertidos
  return resultado;
}
...
function voltearHorizontal(matriz) {
 // 1. Crear una nueva matriz para el resultado
  ...
  // 2. Voltear cada fila de la matriz
  for (let i = 0; i < resultado.length; i++) {
    resultado[i].reverse(); // Usamos reverse() para invertir cada fila horizontalmente
  }

  // 3. Retornar la matriz con las filas volteadas horizontalmente
  return resultado;
}
...
function rotar90Grados(matriz) {
  ...
  // 2. Recorrer las columnas de la matriz original y hacer la rotación
  const alto = matriz.length;
  const ancho = matriz[0].length;

  // 3. Transponer y rotar la matriz
  for (let col = 0; col < ancho; col++) {
    const nuevaFila = [];
    for (let fila = alto - 1; fila >= 0; fila--) {
      nuevaFila.push(matriz[fila][col]);
    }
    resultado.push(nuevaFila);
  }

  // 4. Retornar la nueva matriz rotada 90 grados
  return resultado;
}
...
// 1. Verificar que tengan las mismas dimensiones
  ...
  // 2. Crear una nueva matriz para almacenar el resultado
  const resultado = [];

  // 3. Mezclar los píxeles de ambas imágenes
  for (let i = 0; i < matriz1.length; i++) {
    ...
    for (let j = 0; j < matriz1[i].length; j++) {
      ...
      // Aplicar el factor de mezcla para cada canal (r, g, b)
      const r = Math.round(pixel1.r * (1 - factor) + pixel2.r * factor);
      const g = Math.round(pixel1.g * (1 - factor) + pixel2.g * factor);
      const b = Math.round(pixel1.b * (1 - factor) + pixel2.b * factor);
      const a = Math.round(pixel1.a * (1 - factor) + pixel2.a * factor);  // Mezclar el canal alpha también

      // Guardar el nuevo píxel mezclado en la fila
      filaMezclada.push({ r, g, b, a });
    }
    // Agregar la fila mezclada a la matriz de resultado
    resultado.push(filaMezclada);
  }

  // 4. Retornar la matriz resultante con los píxeles mezclados
  return resultado;
}
